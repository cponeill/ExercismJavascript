...
describe('Acronyms are produced from', function () {
  ...
  it('other title cased phrases', function () {
    expect(new Acronyms().parse('Ruby on Rails')).toEqual('ROR');
  });

  it('inconsistently cased phrases', function () {
    expect(new Acronyms().parse('HyperText Markup Language')).toEqual('HTML');
  });

  it('phrases with punctuation', function () {
    expect(new Acronyms().parse('First In, First Out')).toEqual('FIFO');
  });

  it('other phrases with punctuation', function () {
    expect(new Acronyms().parse('PHP: Hypertext Preprocessor')).toEqual('PHP');
  });

  it('phrases with punctuation and sentence casing', function () {
    expect(new Acronyms().parse('Complementary metal-oxide semiconductor')).toEqual('CMOS');
  });
});
